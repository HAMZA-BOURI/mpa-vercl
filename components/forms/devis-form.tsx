...
import { FileText, User, Car, Calendar as CalendarIcon, Plus, Trash2, Euro, Wrench, PaintBucket } from 'lucide-react'
...
export function DevisForm({ onSubmit, onCancel, clients, vehicules, prestations, isLoading = false }: DevisFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Conditions du Devis</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className={`w-full justify-start text-left font-normal ${errors.dateValidite ? 'border-red-500' : ''}`}
                >
            ...
            {errors.dateValidite && <p className="text-sm text-red-500">{errors.dateValidite}</p>}
          </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="compteBancaire">Compte Bancaire (Optionnel)</Label>
            <Input
              id="compteBancaire"
